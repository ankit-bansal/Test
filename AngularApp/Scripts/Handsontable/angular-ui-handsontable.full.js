...
.directive('uiHandsontable', function ($compile, $rootScope, getHandsontableSettings, $http) {
                ...
                column.source = function (query, process) {
                    if (!column.src) {
                        ...
                        if (angular.isArray(column.src)) {
                            ...
                            column.source = scope.$parent[column.src];
                            machedvals = [];
                            for (var i = 0; i < column.source.length; i++)
                                if (column.source[i].toLowerCase().indexOf(query) !== -1)
                                    machedvals.push(column.source[i]);
                            process(machedvals);
